...
import request, {
    API_SHOP_GOODS_INFO, API_SUBSCRIBE_GOODS_INFO,
    resolveUrl,
...
import {addShopCart, addSubscribeCart} from "../../global";
...
const LiveRoomVerticalPage = (props: any) => {
    const {context} = props;
    const {headerHeight, userInfo} = context;
    ...
    const addCart = (item:any) => {
        ...
        if(liveDetail.goodsType == 1) {
            request.get(API_SHOP_GOODS_INFO+"/"+item.goodsId, SERVICE_WINKT_SYSTEM_HEADER, null, false).then(res=>{
                addShopCart(res.data.data);
            });
        }
        else if(liveDetail.goodsType == 2){
            //加入书袋去借阅书籍，在这之前得验证当前书籍是否和当前用户的会员一致
            if(!userInfo || userInfo.memberInfo.siteId != item.siteId) {
                Taro.showToast({title: '您不是该书籍所在门店的会员，无法借阅', duration: 1500, icon: 'none'}).then();
                return;
            }
            request.get(API_SUBSCRIBE_GOODS_INFO +"/"+item.goodsId, SERVICE_WINKT_SYSTEM_HEADER, null, false).then(res=>{
                addSubscribeCart(res.data.data);
            });
        }
    }
    const openShopCart = () => {
            ...
            <View className={'padding-left-sm padding-right-sm flex flex-direction justify-end'} style={{
                ...
                <View className={'cu-bar tabbar'} style={{width: '100%'}}>
                    <View className={'flex justify-start align-center'} style={{width: '100%'}}>
                        ...
                        {liveDetail.goodsType == 1 &&
                        <Image onClick={openShopCart} className={'margin-left-sm'}
                               src={'../../assets/images/cart.png'}
                               style={{width: Taro.pxTransform(36), height: Taro.pxTransform(36)}}/>
                        }
                        {liveDetail.goodsType == 2 &&
                            <Image onClick={openSubscribeCart} className={'margin-left-sm'}
                        ...
                        return (
                            <View className={classNames("cu-item")} style={{height: '180rpx'}}>
                                ...
                                <View className="action text-sm">
                                    {liveDetail.goodsType == 1 && <Button onClick={()=>addCart(item)} className={'cu-btn sm bg-orange'}>加入购物车</Button>}
                                    {liveDetail.goodsType == 2 && <Button onClick={()=>addCart(item)} className={'cu-btn sm bg-orange'}>加入书袋</Button>}
                                </View>
                            </View>
                        );
                    })}
...
export default withLogin(LiveRoomVerticalPage);
