...
const ShopOrderServices = (props) => {
    ...
    const loadData = (clear = false, page = 1) => {
        ...
        setPage(_page);
        request.get("wxapp/shop/after/orders", SERVICE_WINKT_ORDER_HEADER, {page: _page}, true).then(res => {
        ...
        <PageLayout statusBarProps={{title: '商城售后订单'}} showTabBar={false} loading={loading}>
            {loading && <ContentLoading height='calc(100vh - 218rpx)'/>}
            {
                !isLogin ?
                    ...
                    : (orders.length === 0 ?
                        <Empty title="暂无售后订单" height='calc(100vh - 400rpx)'/> :
                        <View className="cu-card article">{orders.map((item: any) => {
                            return (
                                <Navigator url={`new_service?id=${item.shopOrderGoods.id}`} className="cu-item"
