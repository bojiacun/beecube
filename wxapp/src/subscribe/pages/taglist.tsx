...
const TagSubscribeGoodsList = (props:any) => {
    ...
    const loadData = (clear = false, page = 1) => {
        ...
        setPage(_page);
        request.get(API_SUBSCRIBE_GOODS_LIST, SERVICE_WINKT_SYSTEM_HEADER, {page: _page, tag: params.tag, "site_id":userInfo?.memberInfo?.siteId??0}).then(res=>{
    ...
    return (
        <PageLayout showTabBar={false} statusBarProps={{title: params.tag}}>
...
export default withLogin(TagSubscribeGoodsList);
