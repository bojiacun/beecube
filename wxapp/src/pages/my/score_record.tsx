...
import request, {API_MEMBER_CREDITS2, SERVICE_WINKT_MEMBER_HEADER} from "../../utils/request";
...
const ScoreRecord = (props) => {
    ...
    const loadData = (clear = false, page = 1) => {
        ...
        setPage(_page);
        request.get(API_MEMBER_CREDITS2, SERVICE_WINKT_MEMBER_HEADER, {page: page}, true).then(res=>{
    ...
    return (
        <PageLayout statusBarProps={{title: '大小豆明细'}}>
                    ...
                    return (
                        <View className="cu-item">
                            ...
                            <View className="action text-right" style={{width: '200rpx'}}>
                                {item.score > 0 ? <Text className="text-bold text-red text-xl">+{item.score}</Text>: <Text className="text-bold text-green text-xl">{item.score}</Text>}
                            </View>
                        </View>
                    );
                })}
...
export default withLogin(ScoreRecord);
