...
const SubscribeOrderServices = (props) => {
    ...
    const loadData = (clear = false, page = 1, params: any = {}) => {
        ...
        setPage(_page);
        request.get("wxapp/manager/subscribe/after/orders", SERVICE_WINKT_ORDER_HEADER, {page: _page, ...params}, true).then(res => {
            setLoadingMore(false);
            let list = res.data.data.content;
            list.forEach(o => {
                o.subscribeOrderGoods.images = o.subscribeOrderGoods.images.split(',');
            })
            if (clear) {
                setOver(false);
                setOrders(list);
    ...
    return (
        <PageLayout statusBarProps={{title: '图书损坏申请'}} showTabBar={false} loading={loading}>
            <View className="cu-bar search bg-white">
                <View className="search-form round">
                    <Text className="cuIcon-search"/>
                    <Input type="text" onInput={handleSearchInput} placeholder="搜索订单号、收货人、电话、借阅人" confirm-type="search"/>
                </View>
            </View>
                ...
                !isLogin ?
                    ...
                    : (orders.length === 0 ?
                        ...
                        <View className="cu-card article">{orders.map((item: any) => {
                            return (
                                <Navigator url={"subscribe_service_check?id=" + item.id} key={item.id} className="cu-item"
                                           style={{paddingBottom: 0}}>
                                    <View className="flex align-center padding solid-bottom text-gray justify-between">
                                        <Text> 单号：{item.subscribeOrderGoods.subscribeOrder.ordersn} </Text>
                                        {item.status == 0 && <Text className="text-gray">已拒绝</Text>}
                                        {item.status == 1 && <Text className="text-red">审核中</Text>}
                                    ...
                                    <View className="cu-list menu-avatar">
                                        <View className="cu-item">
                                            <View className="cu-avatar radius lg"
                                                  style={{backgroundImage: 'url(' + resolveUrl(item.subscribeOrderGoods.images[0]) + ')'}}/>
                                            <View className="content" style={{display: 'block', padding: 0}}>
                                                <View className="text-black">
                                                    <Text className="text-cut">{item.subscribeOrderGoods.name}</Text>
                                                </View>
                                            </View>
                                            <View className="action" style={{width: '160rpx'}}>
                                                <View className="text-gray">X {item.subscribeOrderGoods.count}</View>
                                            </View>
                                        </View>
                                    </View>
                                    <View className="flex solid-top padding text-gray justify-between align-center text-sm">
                                        <View className='text-red'>损坏</View>
                                        <View>申请时间：{moment(item.createdAt).format('YYYY-MM-DD HH:mm:ss')}</View>
                                    </View>
                                </Navigator>
                            );
                        })}</View>)
