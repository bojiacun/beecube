...
const EditSubscribeGoods = (props) => {
    ...
    const bookShelfRef = useRef<InputProps>();
    ...
    useReady(() => {
        request.get(API_SUBSCRIBE_GOODS_CLASSES, SERVICE_WINKT_SYSTEM_HEADER, null, false).then(res=>{
            ...
            if(params.id) {
                request.get(API_SUBSCRIBE_GOODS_INFO+ "/" + params.id, SERVICE_WINKT_SYSTEM_HEADER, null, false).then(res => {
                    ...
                    setInfo(detail);
                    setSelectedClassIndex(_.findIndex(shopClasses, {id: detail.subscribeClass.id}));
                    identifierRef.current!.value = detail.identifier;
                    bookShelfRef.current!.value = detail.bookShelf;
    ...
    const onFinish = (e) => {
        ...
        data.subscribeClass = selectedClassIndex > -1 ? {id: classes[selectedClassIndex].id} : null;
        setPosting(true);
        //验证数据
        ...
        makeLogin(() => {
            ...
            if(!info) {
                return request.post("wxapp/manager/subscribe/goods", SERVICE_WINKT_SYSTEM_HEADER, data, true).then(() => {
                    setPosting(false);
                ...
                return request.put("wxapp/manager/subscribe/goods/"+info.id, SERVICE_WINKT_SYSTEM_HEADER, data, true).then(() => {
                    setPosting(false);
    ...
    const handleMarketPriceChange = (e) => {
        ...
        if(val) {
            priceRef.current!.value = (val * 0.6).toFixed(2);
        }
        else {
    ...
    // @ts-ignore
    return (
        <PageLayout statusBarProps={{ title: info ? '编辑借阅图书':'新建借阅图书'}}>
            <Form onSubmit={onFinish}>
                ...
                <View className="cu-form-group">
                    <View className="title"><Text className={'text-red'}>*</Text>书架号</View>
                    <Input name="bookShelf" placeholder="书架号" ref={bookShelfRef} />
                </View>
                <View className="cu-form-group">
                ...
                <View className="cu-form-group">
                    ...
                    <Input name="shortName" placeholder="请输入短标题" ref={shortNameRef} />
                </View>
                <View className="cu-form-group">
                ...
                <View className="cu-form-group">
                    ...
                    <Input name="marketPrice" onInput={handleMarketPriceChange} placeholder="请输入图书定价" type='digit' ref={marketPriceRef} />
                </View>
                <View className="cu-form-group">
...
export default withLogin(EditSubscribeGoods)
