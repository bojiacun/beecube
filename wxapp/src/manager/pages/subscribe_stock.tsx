...
const SubscribeStockRecord = (props) => {
    ...
    const loadData = (clear = false, page = 1) => {
        ...
        setPage(_page);
        request.get("wxapp/manager/subscribe/goods/stocks/" + params.id, SERVICE_WINKT_SYSTEM_HEADER, {page: page}, true).then(res => {
    ...
    return (
        <PageLayout statusBarProps={{title: `【${info?.shortName}】库存记录`}}>
            ...
            <Navigator url={`new_subscribe_stock?id=${info?.id}`} className={'cu-bar tabbar justify-center'} style={{width: '100%', position: 'fixed', bottom: 0, zIndex: 999}}>
...
export default withLogin(SubscribeStockRecord);
