...
import {
    ...
    View,
    RadioGroup,
    Radio, Label, Textarea, Text, Input, Navigator
...
const ServiceCheckPage = (props) => {
    ...
    useReady(() => {
        request.get("wxapp/manager/shop/after/orders/" + params.id, SERVICE_WINKT_ORDER_HEADER, null, true).then(res => {
            setLastOrder(res.data.data);
            setInfo(res.data.data.shopOrderGoods);
            setLoading(false);
        });
    });
    useEffect(() => {
    ...
    const onSubmit = (e) => {
        ...
        makeLogin(() => {
            ...
            return request.put("wxapp/manager/shop/after/orders/"+lastOrder.id+"/confirm", SERVICE_WINKT_ORDER_HEADER, data, true).then((res) => {
                setPosting(false);
                setLastOrder(res.data.data);
        ...
        if (lastOrder && lastOrder.status == 1) {
            return (
                <Form onSubmit={onSubmit} ref={formRef}>
                    <Navigator url={`order_detail?id=${lastOrder.shopOrderGoods.shopOrder.id}`} className="cu-form-group">
                        <View className="title">用户订单</View>
                        <View>
                            {lastOrder.shopOrderGoods.shopOrder.ordersn} <Text className='cuIcon-right' />
                        </View>
                    </Navigator>
                    ...
                    <View className="cu-form-group">
                        ...
                        <RadioGroup name={'resolveType'} onChange={handleResolveTypeChanged} className={'flex align-center'}>
                            <Label className={'margin-right'}>
                                <Radio value={'1'} checked={resolveType == 1} className={'margin-right-xs orange'}/> 审核通过
                            </Label>
                            <Label className={'margin-right'}>
                    ...
                    {resolveType == 2 &&
                        <View className="cu-form-group">
                            <View className="title">拒绝原因</View>
                            <Textarea name="denyNote" placeholder="请填写拒绝申请的原因" />
                        </View>
                    }
                    <Button disabled={posting} loading={posting} formType='submit' className="cu-btn block lg bg-orange shadow"
            ...
            return (
                <Form onSubmit={onSubmit} ref={formRef}>
                    ...
                    <View className='padding bg-white margin-top-sm'>该售后申请已于{moment(lastOrder?.resolvedAt).format('yyyy-MM-DD HH:mm:ss')}{lastOrder?.status == 0 ? '被拒绝': '审核通过'}</View>
                </Form>
            );
        }
    }
...
export default withLogin(ServiceCheckPage)
