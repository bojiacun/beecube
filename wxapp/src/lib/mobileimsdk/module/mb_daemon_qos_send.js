 ...
 */
const MBHashMap = require("../utils/hashmap.js")

/**
 * QoS机制中提供消息送达质量保证的定时器实现类。
 *
 * 本类是QoS机制的核心，目的是加强保证WebSocket协议在应用层的可靠性和送达率。
 *
 ...
 */
class ModuleMBQoS4SendDaemon {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {
    this.TAG = "MBQoS4SendDaemon";
  ...
  doRetryCheck(lostMessages) {
    this._excuting = true;
    try {
      if (wx.MBCore.debugEnabled() && this.sentMessages.size() > 0)
        wx.MBUtils.mblog_d(this.TAG, "【QoS】====== 消息发送质量保证线程运行中, 当前需要处理的列表长度为" + this.sentMessages.size() + "...");
        ...
        if (p && p.QoS) {
          ...
          if (p.retryCount && p.retryCount >= this.QOS_TRY_COUNT) {
            if (wx.MBCore.debugEnabled())
              wx.MBUtils.mblog_d(this.TAG, "【QoS】指纹为" + p.fp + "的消息包重传次数已达" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次)上限，将判定为丢包！");
          ...
          // 没有达到重传上限则开始进行重传
            ...
            var delta = wx.MBUtils.getCurrentUTCTimestamp() - (sendMessageTimestamp == null ? 0 : sendMessageTimestamp);
            // 该消息包是“刚刚”发出的，本次不需要重传它哦
            if (delta <= this.MESSAGES_JUST$NOW_TIME) {
              if (wx.MBCore.debugEnabled())
                wx.MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + key + "的包距\"刚刚\"发出才" + delta + "ms(<=" + this.MESSAGES_JUST$NOW_TIME + "ms将被认定是\"刚刚\"), 本次不需要重传哦.");
            }
            //### 2015103 Bug Fix END
            else {
              var code = wx.MBDataSender.sendCommonData(p);
              //已成功重传
              if (code === 0) {
                // 重传次数+1
                if (!p.retryCount) {
                ...
                if (wx.MBCore.debugEnabled())
                  wx.MBUtils.mblog_d(this.TAG, "【QoS】指纹为" + p.fp + "的消息包已成功进行重传，此次之后重传次数已达" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次).");
              } else {
                wx.MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + p.fp + "的消息包重传失败，它的重传次数之前已累计为" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次).");
              }
            }
          }
        }
      ...
      wx.MBUtils.mblog_w(this.TAG, "【QoS】消息发送质量保证线程运行时发生异常：", eee);
    }
  ...
  notifyMessageLost(lostMessages) {
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_MESSAGE_LOST, lostMessages);
  }
      ...
      if (p.fp) {
        if (p.QoS) {
          ...
          if (this.sentMessages.get(p.fp) != null)
            wx.MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + p.fp + "的消息已经放入了发送质量保证队列，该消息为何会重复？（生成的指纹码重复？还是重复put？）");

          // save it
          this.sentMessages.put(p.fp, p);
          // 同时保存时间戳
          this.sendMessagesTimestamp.put(p.fp, wx.MBUtils.getCurrentUTCTimestamp());
        } else {
          wx.MBUtils.mblog_w(this.TAG, "This protocal is not QoS pkg, ignore it!");
        }
      } else {
        wx.MBUtils.mblog_w(this.TAG, "Invalid arg p.getFp() == null.");
      }
    } else {
      wx.MBUtils.mblog_w(this.TAG, "Invalid arg p==null.");
    }
  }
  ...
  remove(fingerPrint) {
    ...
    wx.MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + fingerPrint + "的消息已成功从发送质量保证队列中移除(可能是收到接收方的应答也可能是达到了重传的次数上限)，重试次数=" + (removedObj != null ? (removedObj.retryCount) : "none呵呵."));
  }
...
// 以全局单例的形式存在，方便使用
wx.MBQoS4SendDaemon = new ModuleMBQoS4SendDaemon();