 ...
 */
class ModuleMBQoS4ReciveDaemon {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {
    this.TAG = "MBQoS4ReciveDaemon";
  ...
  excute() {
    ...
    if (!this._excuting) {
      ...
      if (wx.MBCore.debugEnabled())
        wx.MBUtils.mblog_d(this.TAG, "【QoS接收方】+++++ START 暂存处理线程正在运行中，当前长度" + this.recievedMessages.size() + ".");
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        var delta = wx.MBUtils.getCurrentUTCTimestamp() - (recievedTime ? recievedTime : 0);
        ...
        if (delta >= this.MESSAGES_VALID_TIME) {
          if (wx.MBCore.debugEnabled())
            wx.MBUtils.mblog_d(this.TAG, "【QoS接收方】指纹为" + key + "的包已生存" + delta + "ms(最大允许" + this.MESSAGES_VALID_TIME + "ms), 马上将删除之.");
    ...
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "【QoS接收方】+++++ END 暂存处理线程正在运行中，当前长度" + this.recievedMessages.size() + ".");
  ...
  addRecieved(fingerPrintOfProtocal) {
    if (fingerPrintOfProtocal) {
      if (this.recievedMessages.contains(fingerPrintOfProtocal))
        wx.MBUtils.mblog_w(this.TAG, "【QoS接收方】指纹为" + fingerPrintOfProtocal +
      ...
      wx.MBUtils.mblog_w(this.TAG, "无效的 fingerPrintOfProtocal==null!");
    }
  }

  putImpl(fingerPrintOfProtocal) {
    if (fingerPrintOfProtocal)
      this.recievedMessages.put(fingerPrintOfProtocal, wx.MBUtils.getCurrentUTCTimestamp());
  }
   ...
   */
  setMessageValidTime = function (time) {
    this.MESSAGES_VALID_TIME = time;
  }
}

// 以全局单例的形式存在，方便使用
wx.MBQoS4ReciveDaemon = new ModuleMBQoS4ReciveDaemon();