...
const SmtRecordList = (props: any) => {
    const {show, onHide, selectedRow} = props;
    const [list, setList] = useState<any>({records: []});
    const [searchState, setSearchState] = useState<any>({...DefaultListSearchParams, templateId: selectedRow.id});
    const searchFetcher = useFetcher();

    useEffect(()=>{
        if(show) {
            searchFetcher.submit(searchState, {method: 'get', action: '/paimai/smts/records'});
        }
    }, [show]);
    ...
    return (
        ...
        >
            <Modal.Header closeButton>
                <Modal.Title id={'edit-modal'}>短信模板发送记录</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <div className={'m-2'}>
                    <Row>
                        ...
                        <Col md={6} className={'d-flex align-items-center justify-content-end'}>
                            <searchFetcher.Form action={'/paimai/invites'} className={'form-inline justify-content-end'}
                                ...
                                <FormGroup as={Row} className={'mb-0'}>
                                    <FormLabel column htmlFor={'memberPhone'}>手机号</FormLabel>
                                    <Col md={'auto'}>
                                        <InputGroup>
                                            <FormControl name={'memberPhone'} autoComplete={'off'} onChange={handleOnSearchNameChanged}
