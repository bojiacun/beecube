...
const IntegralGoodsClassEditor = (props: any) => {
    ...
    const handleOnSubmit = (values: any) => {
        if (values.id) {
            postFetcher.submit(values, {method: 'post', action: '/paimai/integral/classes/edit'});
        } else {
            postFetcher.submit(values, {method: 'post', action: '/paimai/integral/classes/add'});
        }
    }
    ...
    return (
        <>
            ...
            >
                ...
                {model &&
                    <Formik innerRef={formikRef} initialValues={{status: 1,sortNum:0,...model}} validationSchema={GoodsClassSchema} onSubmit={handleOnSubmit}>
                        {(formik)=>{
                            return (
                                <Form method={'post'}>
...
export default IntegralGoodsClassEditor;