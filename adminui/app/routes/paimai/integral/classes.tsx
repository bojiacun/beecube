...
import {API_PAIMAI_INTEGRAL_GOODS_CLASS_LIST, requestWithToken} from "~/utils/request.server";
import {withPageLoading} from "~/utils/components";
import IntegralGoodsClassList from "~/pages/paimai/IntegralGoodsClassList";
...
export const loader: LoaderFunction = async ({request}) => {
    ...
    const result = await requestWithToken(request)(API_PAIMAI_INTEGRAL_GOODS_CLASS_LIST+ queryString);
    return json(result.result);
}

const GoodsClassesPage = (props:any) => {
    return (
        <IntegralGoodsClassList {...props} />
    );
}
