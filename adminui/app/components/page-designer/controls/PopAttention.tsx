...
export const POP_ATTENTION = "POP_ATTENTION";
...
const PopAttentionAttributeView : React.FC<any> = (props) => {
                        ...
                        (formik) => {
                            return (
                                <Form method={'post'} onChange={(e)=>formik.submitForm()}>
                                    <FormGroup>
                                        <FormLabel htmlFor={'image'}>公众号二维码</FormLabel>
    ...
    return (
        <div {...rest}  style={{width: '100%', height: '100%', top: 0, left: 0, position: 'absolute', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: `rgba(0,0,0,${_data.style.opacity})`}}>
            <div style={{width: '50%', position: 'relative', opacity: 1}}>
                <img alt={'pop_Attention'} src={resolveUrl(_data.basic.image)} style={{width: '100%', display: 'block'}} />
...
export default function () {
    registerControl(POP_ATTENTION, false, "关注公众号", PopAttention, PopAttentionAttributeView, defaultData);
}
