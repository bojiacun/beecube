...
import {getPagedGoods, getPagedPerformance, getPagedTagedPerformance, getPagedTagedRooms, getShopClasses} from "./service";
...
export const LIVE_ROOM_LIST_MODULE = "LIVE_ROOM_LIST_MODULE";
...
const LiveRoomListModuleAttribute = (props: any) => {
                        ...
                        (formik) => {
                            return (
                                <Form method={'post'} onChange={(e) => formik.submitForm()}>
                                    <BootstrapRadioGroup
                                        options={[{label: '进行中直播', value: '1'}, {label: '往期直播', value: '2'}]}
                                        name={'dataSource'}
                                        label={'数据类型'}
                                    />
...
const LiveRoomListModule = (props: any) => {
    ...
    const [goodsList, setRoomList] = useState<any[]>([]);
    let _data = {...defaultData, ...data};
    useEffect(() => {
        if (_data.basic.dataSource == 1) {
            //最新商品
            getPagedTagedRooms(_data.basic.tag, 1, _data.basic.count).then(res => {
                setRoomList(res.data.records);
            });
        } else if (_data.basic.dataSource == 2) {
            getPagedTagedRooms(_data.basic.tag, 2, _data.basic.count).then(res => {
                setRoomList(res.data.records);
            });
        } else {
            setRoomList([]);
...
export default function (module = '') {
    if (module === 'paimai') {
        registerModule(LIVE_ROOM_LIST_MODULE, "直播列表", image, '拍卖模块', LiveRoomListModule, LiveRoomListModuleAttribute, defaultData);
    }
}