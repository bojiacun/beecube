...
export async function getPagedPerformance(type: number,source:any, count:number) {
    return axios.get('/paimai/performance/diy/list', { params: {type: type, source: source, pageSize: count}});
}
export async function getPagedAuction(count:number) {
    return axios.get('/paimai/auction/diy/list', { params: {pageSize: count}});
}
