...
export default class Index extends Component<any, any> {
    ...
    loadData(id, page, clear = false) {
        return request.get('/paimai/api/members/follows', {params: {id: id, pageNo: page, column: 'createTime', order: 'desc'}}).then(res=>{
            if(clear) {
                        ...
                        {list.map((item)=>{
                            ...
                            return (
                                <View className={'bg-white relative overflow-hidden'} style={{borderRadius: Taro.pxTransform(radius)}}>
                                    <Navigator url={(item.type == 1 ? '/pages/goods/detail?id=':'/pages/goods/detail2?id=')+item.id}>
                                        <View className={'relative'} style={{width: '100%', paddingTop: '100%'}}>
                                            <FallbackImage mode={'aspectFill'} className={'absolute z-0 inset-0 block w-full h-full'} src={utils.resolveUrl(item.images.split(',')[0])}/>
                                        </View>
                                        <View className={'px-2 mt-2'}>{item.title}</View>
                                        <View className={'px-2 mb-2 text-sm'}>
                                            当前价 <Text className={'text-red-500'}>RMB</Text> <Text className={'text-red-500 text-lg'}>{numeral(item.currentPrice||item.startPrice).format('0,0.00')}</Text>
                                        </View>
                                    </Navigator>
                                </View>
                            );
