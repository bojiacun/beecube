...
export default class Index extends Component<any, any> {
    ...
    loadData(pageIndex: number, tab: ListViewTabItem) {
        let params: any = {type: 2, column: 'createTime', order: 'desc', pageNo: pageIndex};
        if (tab.id) {
            params.tabId = tab.id;
        }
        ...
        return (
            <PageLayout statusBarProps={{title: settings.articleVideoIndexTitle || '视频类文章频道页'}} enableReachBottom={true}>
                <ListView
                    ...
                    showSearch={true}
                    searchUrl={'/article/pages/search?title='+encodeURIComponent(settings.articleVideoIndexTitle||'视频类文章频道页')}
                />
            </PageLayout>
        );
    }
