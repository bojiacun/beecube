 ...
 * The 微 Project. All rights reserved.
 *
 ...
 */
/**
 * MobileIMSDK-微信小程序端框架的核心算法层入口类（相当于APP客户端sdk中的ClientCoreSDK类）。
 * <br>
 * 本类主要提供一些全局参数的读取和设置。
 *
...
class ModuleMBCore {
  ...
  getEventCallback(eventName) {
    if (eventName) {
      ...
      wx.MBUtils.mblog_d(this.TAG, "事件监听者列表中，名称为 eventName=" + eventName + "的callback 是空的！");
      return null;
    } else
      wx.MBUtils.mblog_w(this.TAG, '无效的参数：eventName=' + eventName);
  }
  ...
  on(eventName, callback) {
    if (eventName && callback) {
      ...
      wx.MBUtils.mblog_w(this.TAG, '无效的参数：eventName=' + eventName + ", callback=" + callback);
    }
  }
  ...
  emit(eventName, data) {
    ...
    if (callbackForEvent) {
      ...
      wx.MBUtils.mblog_w(this.TAG, '事件回调列表中没有找到eventName=' + eventName + "的callback，本次emit无法进行！");
    }
  }
  ...
  release() {
    ...
    wx.MBQoS4SendDaemon.clear();
    // 并清除QoS接收队列缓存：防止不退出APP时切换另一账号后qos的缓存队列未清空
    wx.MBQoS4ReciveDaemon.clear();
