 ...
 */
/**
 * 数据接收辅助类（因微信小程序websocket的数据接收不需要像经典的BIO那样需开发者自已启动单独
 ...
 */
class ModuleMBDataReciever {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {
    this.TAG = "MBDataReciever";
  }
    ...
    if (pFromServer) {
      ...
      try {
        ...
        if (pFromServer.QoS) {
          ...
          if (pFromServer.type === wx.MBProtocalType.FROM_SERVER_TYPE_OF_RESPONSE$LOGIN &&
            JSON.parse(pFromServer.dataContent).code !== 0) {
            if (wx.MBCore.debugEnabled())
              wx.MBUtils.mblog_d(this.TAG, "【BugFIX】这是服务端的登陆返回响应包，且服务端判定登陆失败(即code!=0)，本次无需发送ACK应答包！");
          }
          // # Bug FIX 20170620 END 【1/2】
          else {
            // 且已经存在于接收列表中（及意味着可能是之前发给对方的应答包因网络或其它情况丢了，对方又因QoS机制重新发过来了）
            if (wx.MBQoS4ReciveDaemon.hasRecieved(pFromServer.fp)) {
              if (wx.MBCore.debugEnabled())
                wx.MBUtils.mblog_d(this.TAG, "【QoS机制】" + pFromServer.fp + "已经存在于发送列表中，这是重复包，通知应用层收到该包罗！");

              //----------------------------------------- [1]代码与[2]处相同的哦 S
              // 【【C2C、C2S、S2C模式下的QoS机制2/4步：将收到的包存入QoS接收方暂存队列中（用于防重复）】】
              wx.MBQoS4ReciveDaemon.addRecievedProtocal(pFromServer);
        ...
        switch (pFromServer.type) {
          // ** 收到通用数据
          case wx.MBProtocalType.FROM_CLIENT_TYPE_OF_COMMON$DATA: {
            this.onRecievedCommonData(pFromServer);
            break;
          }
          //** 收到服务反馈过来的心跳包
          case wx.MBProtocalType.FROM_SERVER_TYPE_OF_RESPONSE$KEEP$ALIVE: {
            this.onServerResponseKeepAlive();
            break;
          }
          //** 收到好友发过来的QoS应答包
          case wx.MBProtocalType.FROM_CLIENT_TYPE_OF_RECIVED: {
            this.onMessageRecievedACK(pFromServer);
            break;
          }
          // ** 收到服务端反馈过来的登陆完成信息
          case wx.MBProtocalType.FROM_SERVER_TYPE_OF_RESPONSE$LOGIN: {
            this.onServerResponseLogined(pFromServer);
            break;
          }
          // ** 服务端返回来过的错误消息
          case wx.MBProtocalType.FROM_SERVER_TYPE_OF_RESPONSE$FOR$ERROR: {
            this.onServerResponseError(pFromServer);
            break;
          }
          // ** 服务端发回来过的“重复登录被踢”消息
          case wx.MBProtocalType.FROM_SERVER_TYPE_OF_KICKOUT: {
            this.onKickout(pFromServer);
            break;
          }
          default: {
            wx.MBUtils.mblog_w(this.TAG, "收到的服务端消息类型：" + pFromServer.getType() + "，但目前该类型客户端不支持解析和处理！");
            break;
          }
        }
      } catch (e) {
        wx.MBUtils.mblog_w(this.TAG, "处理消息的过程中发生了错误.", e);
      }
    } else {
      wx.MBUtils.mblog_d(this.TAG, "无效的pFromServer（" + pFromServer + "）！");
    }
  }
  ...
  onRecievedCommonData(pFromServer) {
    ...
    // 收到通用数据的回调
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_ON_RECIEVE_MESSAGE, pFromServer);
  }
  ...
  onServerResponseKeepAlive() {
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "收到服务端回过来的Keep Alive心跳响应包.");

    // 给应用层发出pong事件通知
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_PONG, null);

    // 更新服务端的最新响应时间（该时间将作为计算网络是否断开的依据）
    wx.MBKeepAliveDaemon.updateGetKeepAliveResponseFromServerTimstamp();
  }
  ...
  onMessageRecievedACK(pFromServer) {
    ...
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "【QoS】收到" + pFromServer.from + "发过来的指纹为" + theFingerPrint + "的应答包.");

    // 将收到的应答事件通知事件处理者
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_MESSAGE_BE_RECIEVED, theFingerPrint);

    // 【【C2C或C2S模式下的QoS机制4/4步：收到应答包时将包从发送QoS队列中删除】】
    wx.MBQoS4SendDaemon.remove(theFingerPrint);
  }
  ...
  onServerResponseLogined(pFromServer) {
    ...
    if (loginInfoRes.code === 0) {
      ...
      if (!wx.MBCore.isLoginHasInit()) {
        wx.MBUtils.mblog_d(this.TAG, "首次登陆验证成功，firstLoginTime=" + loginInfoRes.firstLoginTime);
        // 将返回的“首次登陆时间”保存，该参数仅用于掉线重连时提交给服务端（服务端多端互踢判定逻辑中会用到）
        wx.MBCore.setCurrenetLoginInfo_firstLoginTime(loginInfoRes.firstLoginTime);
      }

      this.fireConnectedToServer();
    } else {
      wx.MBUtils.mblog_d(this.TAG, "登陆验证失败，错误码=" + loginInfoRes.code + "！");
      ...
      // 登陆失败后关闭网络监听是合理的作法
      wx.MBSocketProvider.closeLocalSocket();

      // 设置中否正常连接（登陆）到服务器的标识（注意：在要event事件通知前设置哦，因为应用中是在event中处理状态的）
      wx.MBCore.setConnectedToServer(false);
    }

    // 用户登陆认证情况事件通知
    // MBCore.emit(MBSocketEvent.SOCKET_EVENT_ON_LOGIN_RESPONSE, loginInfoRes.code);
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_ON_LOGIN_RESPONSE, loginInfoRes);
  }
  ...
  onServerResponseError(pFromServer) {
    ...
    if (errorRes.errorCode === wx.MBErrorCode.RESPONSE_FOR_UNLOGIN) {
      //
      wx.MBCore.setLoginHasInit(false);

      wx.MBUtils.mblog_e(this.TAG, "收到服务端的“尚未登陆”的错误消息，心跳线程将停止，请应用层重新登陆.");
      // 停止心跳
      wx.MBKeepAliveDaemon.stop();
      ...
      wx.MBAutoReLoginDaemon.start(false);
    }

    // 收到错误响应消息的回调
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_ON_ERROR_RESPONSE, errorRes);
  }
  ...
  onKickout(pFromServer) {
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "收到服务端发过来的“重复登录被踢”消息.");
    ...
    /* 给应用层发出“被踢”事件通知 */
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_KICKOUT, kickoutInfo);
    ...
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_ON_LINK_CLOSE, -1);
  }
  ...
  fireConnectedToServer() {
    ...
    // 标记已完成首次登陆服务端
    wx.MBCore.setLoginHasInit(true);

    // 尝试关闭自动重新登陆线程（如果该线程正在运行的话）
    wx.MBAutoReLoginDaemon.stop();
    ...
    // 立即开启Keepalive心跳线程
    wx.MBKeepAliveDaemon.setNetworkConnectionLostCallback(networkConnectionLostCallBack);

    // ** 2015-02-10 by Jack Jiang：收到登陆成功反馈后，无需立即就发起心跳，因为刚刚才与服务端
    // ** 成功通信了呢（刚收到服务器的登陆成功反馈），节省1次心跳，降低服务重启后的“雪崩”可能性
    wx.MBKeepAliveDaemon.start(false);

    //【登陆成功后开启QoS机制】启动QoS机制之发送列表重视机制
    wx.MBQoS4SendDaemon.startup(true);
    // 启动QoS机制之接收列表防重复机制
    wx.MBQoS4ReciveDaemon.startup(true);
    // 设置中否正常连接（登陆）到服务器的标识（注意：在要event事件通知前设置哦，因为应用中是在event中处理状态的）
    wx.MBCore.setConnectedToServer(true);
  }
  ...
  fireDisconnectedToServer() {
    ...
    wx.MBQoS4ReciveDaemon.stop();

    // 通知回调实现类
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_ON_LINK_CLOSE, -1);
    // 网络断开后即刻开启自动重新登陆线程从而尝试重新登陆（以便网络恢复时能即时自动登陆）
    wx.MBAutoReLoginDaemon.start(true); // 建议：此参数可由true改为false，防止服务端重启等情况下，客户端立即重连等
  }
  ...
  sendRecievedBack(pFromServer) {
    if (pFromServer.fp) {
      ...
      var pForRecivedBack = wx.MBProtocalFactory.createRecivedBack(pFromServer.to, pFromServer.from, pFromServer.fp);
      var code = wx.MBDataSender.sendCommonData(pForRecivedBack);

      if (wx.MBCore.debugEnabled())
        wx.MBUtils.mblog_d(this.TAG, "【QoS】向" + pFromServer.from + "发送" + pFromServer.fp + "包的应答包成功？(code=" + code + "),from=" + pFromServer.to + "！");
    } else {
      wx.MBUtils.mblog_w(this.TAG, "【QoS】收到" + pFromServer.from + "发过来需要QoS的包，但它的指纹码却为null！无法发应答包！");
    }
  }
}

// 以全局单例的形式存在，方便使用
wx.MBDataReciever = new ModuleMBDataReciever(); 