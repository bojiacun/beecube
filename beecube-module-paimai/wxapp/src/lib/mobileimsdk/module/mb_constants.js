...
// 以全局常量的形式存在，方便使用
wx.MBProtocalType = MBProtocalType;
...
// 以全局常量的形式存在，方便使用
wx.MBErrorCode = MBErrorCode;

/**
* Socket事件名定义。
*
* MobileIMSDK-微信小程序端框架中，各种网络事件是通过事件通知的方法抛出，从而与主逻辑解耦，
...
// 以全局常量的形式存在，方便使用
wx.MBSocketEvent = MBSocketEvent;
...
// 以全局常量的形式存在，方便使用
wx.MBKickoutCode = MBKickoutCode;
...
*/
class ModuleMBProtocalFactory{

  // 构造器（相当于java里的构造方法）
  constructor(argument){
      this.MSG_ID_LENGTH = 24;
  }

  /**
   * 设置生成的消息id长度。
   * 
   * @param {Number} len 建议设置32，不设置则默认值24
   */
  setMsgIdLength(len) {
    if(len && len > 0) {
      this.MSG_ID_LENGTH = len;
    } else {
      console.error('无效的消息id生成长度设置：len='+len);
    }
  }
   ...
   * @return {Protocal}
   */
  createProtocal(type, dataContent, from, to, QoS, fingerPrint, typeu) {
      ...
      p.QoS = QoS;
      // 只有在需要QoS支持时才生成指纹，否则浪费数据传输流量目前一个包的指纹只在对象建立时创建哦
      if(QoS) {
          p.fp = (fingerPrint ? fingerPrint : wx.MBUtils.uuid(this.MSG_ID_LENGTH));//32//uuid.v1());
      }
   ...
   * @see #createProtocal()
   */
  createCommonData(dataContent, from_user_id, to_user_id, QoS, fingerPrint, typeu){
   ...
   * @see #createProtocal()
   */
  createCommonDataSimple(dataContent, from_user_id, to_user_id, typeu){
...
// 以全局单例的形式存在，方便使用
wx.MBProtocalFactory = new ModuleMBProtocalFactory();
