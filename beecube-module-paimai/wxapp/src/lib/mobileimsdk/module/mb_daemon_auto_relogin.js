 ...
 */
class ModuleMBAutoReLoginDaemon {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {

    this.TAG = "MBAutoReLoginDaemon";
  ...
  setAutoReloginInterval(interval) {
    if (wx.MBUtils.isPositiveInteger(interval))
      this.AUTO_RE$LOGIN_INTERVAL = interval;
    else
      wx.MBUtils.mblog_w(this.TAG, "无效的AUTO_RE$LOGIN_INTERVAL参数设置：" + interval);
  }
  ...
  doSendLogin() {
    ...
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "自动重新登陆线程执行中, autoReLogin ? " + wx.MBCore.autoReLogin + "...");

    let code = -1;
    // 是否允许自动重新登陆哦
    if (wx.MBCore.isAutoRelogin()) {
      code = wx.MBDataSender.sendLogin(wx.MBCore.getCurrentLoginInfo());
    }
    return code;
  }
  ...
  onSendLogin(result) {
    // 给应用层发出尝试重新登陆/连接事件通知
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_RECONNECT_ATTEMPT, null);
    if (result === 0) {
      // do nothing
    }
    this._excuting = false;
...
// 以全局单例的形式存在，方便使用
wx.MBAutoReLoginDaemon = new ModuleMBAutoReLoginDaemon();