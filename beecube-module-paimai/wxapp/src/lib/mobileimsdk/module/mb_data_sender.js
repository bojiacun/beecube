...
/**
 * 数据发送处理实用类。
 * 本类是MobileIMSDK-微信小程序端框架的唯一提供数据发送的公开实用类。
 *
 * @author Jack Jiang(http://www.52im.net/thread-2792-1-1.html)
 */
class ModuleMBDataSender {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {
    this.TAG = "MBDataSender";
  }
  ...
  sendLogin(loginInfo) {
    ...
    if (!wx.MBSocketProvider.isLocalSocketReady()) {

      if (wx.MBCore.debugEnabled())
        wx.MBUtils.mblog_d(this.TAG, "发送登陆指令时，socket连接未就绪，首先开始尝试发起连接（登陆指令将在连接成功后的回调中自动发出）。。。。");
      ...
      let connectionDoneCallback = function (sucess, extraObj) {
        ...
        // TCP连接建立失败
          ...
          wx.MBUtils.mblog_w(this.TAG, "[来自ws连接结果回调观察者通知]socket连接失败，本次登陆信息未成功发出！");
      };
      // 调置连接回调
      wx.MBSocketProvider.setConnectionDoneCallback(connectionDoneCallback);

      // 主要是为了触发 LocalUDPSocketProvider中的 tryConnectToHost()调用
      // ，connect完后将自动调用上方设置的回调，从而实现登陆指令在连接成功后的发送
      return wx.MBSocketProvider.resetLocalSocket() != null ?
        wx.MBErrorCode.COMMON_CODE_OK : wx.MBErrorCode.BAD_CONNECT_TO_SERVER;
    } else {
      // 发送登陆指令
  ...
  sendLoginImpl(loginInfo) {
    let p = wx.MBProtocalFactory.createPLoginInfo(loginInfo.loginUserId, loginInfo);
    let code = this.send(JSON.stringify(p));    
    // 登陆信息成功发出时就把登陆名存下来
    if (code === 0) {
      wx.MBCore.setCurrenetLoginInfo(loginInfo);
    }
    return code;
  }
   ...
   */
  sendLoginout() {
    let code = wx.MBErrorCode.COMMON_CODE_OK;
    if (wx.MBCore.isLoginHasInit()) {
      let p = wx.MBProtocalFactory.createPLoginoutInfo(wx.MBCore.getCurrentLoginInfo().loginUserId);
   ...
   */
  sendKeepAlive() {
    let p = wx.MBProtocalFactory.createPKeepAlive(wx.MBCore.getCurrentLoginInfo().loginUserId);
   ...
   * @see #send()
   */
  sendCommonData(p, fnSuccess, fnFail, fnComplete) {
    if (p) {
      // byte[] b = p.toBytes();
      let dataWithString = JSON.stringify(p);
      let code = this.send(dataWithString, fnSuccess, fnFail, fnComplete);
      if (code === 0) {
        // 【【C2C或C2S模式下的QoS机制1/4步：将包加入到发送QoS队列中】】
        // 如果需要进行QoS质量保证，则把它放入质量保证队列中供处理(已在存在于列表中就不用再加了，已经存在则意味当前发送的这个是重传包哦)
        if (p.QoS && !wx.MBQoS4SendDaemon.exist(p.fp))
          wx.MBQoS4SendDaemon.put(p);
      }
      return code;
    } else
      return wx.MBErrorCode.COMMON_INVALID_PROTOCAL;
  }
   ...
   * @return {int} 0表示数据发出成功，否则返回的是错误码
   */
  send(dataWithString, fnSuccess, fnFail, fnComplete) {
    // 获得netty的channel实例
    let websocket = wx.MBSocketProvider.getLocalSocket();

    // 关于WebSocket的readyState字段值含义，请见：
    // https://developer.mozilla.org/zh-CN/docs/Web/API/WebSocket/readyState
    if (websocket != null && websocket.readyState === 1) {
      return wx.MBUtils.send(websocket, dataWithString, fnSuccess, fnFail, fnComplete) ? wx.MBErrorCode.COMMON_CODE_OK : wx.MBErrorCode.COMMON_DATA_SEND_FAILD;
    } else {
      ...
      wx.MBUtils.mblog_d(this.TAG, "scocket未连接，无法发送，本条将被忽略（dataWithString=" + dataWithString + "）!");
      return wx.MBErrorCode.COMMON_CODE_OK;
    }
  }
}

// 以全局单例的形式存在，方便使用
wx.MBDataSender = new ModuleMBDataSender(); //MBDataSender;