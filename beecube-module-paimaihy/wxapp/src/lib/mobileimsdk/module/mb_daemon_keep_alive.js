 ...
 */
class ModuleMBKeepAliveDaemon {

  // 构造器（相当于java里的构造方法）
  constructor(argument) {
    this.TAG = "MBKeepAliveDaemon";
  ...
  doKeepAlive() {
    ...
    if (wx.MBCore.debugEnabled())
      wx.MBUtils.mblog_d(this.TAG, "心跳线程执行中...");
    return wx.MBDataSender.sendKeepAlive();
  }
  ...
  onKeepAlive(code) {
    // 给应用层发出ping事件通知
    wx.MBCore.emit(wx.MBSocketEvent.SOCKET_EVENT_PING, null);
    ...
    if (isInitialedForKeepAlive)
      this.lastGetKeepAliveResponseFromServerTimstamp = wx.MBUtils.getCurrentUTCTimestamp();

    // 首先启动心跳时就不判断了，否则就是逻辑有问题
    if (!isInitialedForKeepAlive) {
      let now = wx.MBUtils.getCurrentUTCTimestamp();
      ...
      if (now - this.lastGetKeepAliveResponseFromServerTimstamp >= this.NETWORK_CONNECTION_TIME_OUT) {
        if (wx.MBCore.debugEnabled()) {
          wx.MBUtils.mblog_d(this.TAG, ">>>> t1=" + now + ", t2=" + this.lastGetKeepAliveResponseFromServerTimstamp +
            " -> 差：" + (now - this.lastGetKeepAliveResponseFromServerTimstamp));
          wx.MBUtils.mblog_d(this.TAG, "心跳机制已判定网络断开，将进入断网通知和重连处理逻辑 ...");
        }
   ...
   *
   * @param immediately {boolean} true表示立即执行线程作业，否则直到 {@link #KEEP_ALIVE_INTERVAL}执行间隔的到来才进行首次作业的执行。
   */
  start(immediately) {
    this.stop();
...
// 以全局单例的形式存在，方便使用
wx.MBKeepAliveDaemon = new ModuleMBKeepAliveDaemon();